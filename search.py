...
def greedy_search(start_node, pellet_node): #Ý tưởng greedy search là tới đâu tìm tới đó nên dùng stack + priority queue sẽ tối ưu
    counter=0
    visited=set()
    stack = [(start_node,[])]
    while stack:
...
def depth_first_search_D(start_node, pellet_node):
    max_depth = 10
    stack =[(start_node,[])]
    visited = set()
    while True:
